...
void EnvironmentScene(const char* filename, CoordMapper* spheremapper1, CoordMapper* spheremapper2)
{
    static const float scale = 0.001f;
    Image img(400, 400);
    World world;
    SimpleGroup scene;
    world.scene = &scene;
    Point reflectiveSphere(400.f, 400.f, 300.f);
    ImageTexture *envtex;
    ...
    scene.add(new Sphere(Point(200.f, 100.f, 300.f)*scale, 150.f*scale, spheremapper2, &white));
    RayTracingIntegrator integrator(&world);
    Image imgtex(512, 512);
    {
        EnvironmentCamera ecam(reflectiveSphere*scale, Vector(0, 0, 1), Vector(0, 1, 0));
        Renderer r4(&ecam, &integrator);
        r4.render(imgtex);
        //r4.test_render4(imgtex);
        envtex = new ImageTexture(imgtex);
    }
    FlatMaterial flatMaterial(envtex);
    
    scene.add(new SpecialSolid(reflectiveSphere*scale, 150.f*scale, spheremapper1, &flatMaterial));
...
void a_mappers() {
    ...
    trymapper("map-3.png",
        new PlaneCoordMapper(Point(0,0,0),Vector(0.25f,0.0f,0.25f),Vector(-0.25f,0.0f,0.25f)),
        new PlaneCoordMapper(Point(0, 0, 0), Vector(0.25f, 0.35f, -0.25f), Vector(-0.25f, 0.35f, -0.25f))
        );
    ...
    EnvironmentScene("map-6.png",
        new EnvironmentCoordMapper(),
        new SphericalCoordMapper(Point(.3f, .1f, .3f), Vector(0, hsq2, -hsq2), Vector(0.5f, 0.0f, 0.0f))
        );
}
