...
void trymapper2(const char* filename, CoordMapper* spheremapper1, CoordMapper* spheremapper2) {
    ...
    Sphere *sph = new Sphere(Point(200.f, 100.f, 300.f)*scale, 150.f*scale, spheremapper2, &white);
    sph->SetCenter2(Point(250.f, 100.f, 300.f)*scale);
    scene.add(sph);

    RayTracingIntegrator integrator(&world);

    Renderer engine(&cam, &integrator);
    engine.test_motionblur(img);
    img.writePNG(filename);
}

void a_blur() {
    float hsq2 = 0.5f / std::sqrt(2.0f);
    trymapper2("blur-1.png",
